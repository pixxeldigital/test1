	            ...
	            <table class="table table-striped" id="datatable-editable">
	                ...
	                <tbody>
	                    ...
	                    <tr class="gradeX">
	                        ...
	                        <td>{{ $branch->status }}</td>
	                        <td class="actions">
	                            <a href="#" class="hidden on-editing save-row"><i class="fa fa-save" id="save-{{$branch->id}}" onclick="return saveData({{$branch->id}})"></i></a>
	                            <a href="#" class="hidden on-editing cancel-row"><i class="fa fa-times"></i></a>
	                            <a href="#" class="on-default edit-row"><i class="fa fa-pencil"></i></a>
	                            <a href="#" class="on-default remove-row" id="delete-{{$branch->id}}" onclick="return deleteData({{$branch->id}})"><i class="fa fa-trash-o"></i></a>
	                        </td>
	                    </tr>
	                   @endforeach
	                </tbody>
...
function saveData(id)
{
	...
	var url = "http://192.168.0.27/loom/public/branch/" +id;
	var name = $('#data_0').val();
	var code = $('#data_1').val();
	var status = $('#data_2').val();
				...
				url: url,
				data: {name:name, code: code, status: status, _method:"PUT"},
...
/* Pull the values and write ajax here */
	var url = "http://192.168.0.27/loom/public/branch";
	var name = $('#data_0').val();
	var code = $('#data_1').val();
	var status = $('#data_2').val();
if((name == '') || (code == '') || (status == ''))  {
	...
	var data = {name:name, code: code, status: status, _method:"POST"};
				 $.ajax({
				type: "POST",
				headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
				url: url,
