	            ...
	            <table class="table table-striped" id="datatable-editable">
	                ...
	                <tbody>
	                    ...
	                    <tr class="gradeX">
	                        ...
	                        <td>{{ $component->default_qty }}</td>
	                        <td>{{ $component->status }}</td>
	                        <td class="actions">
	                            <a href="#" class="hidden on-editing save-row"><i class="fa fa-save" id="save-{{$component->id}}" onclick="return saveData({{$component->id}})"></i></a>
	                            <a href="#" class="hidden on-editing cancel-row"><i class="fa fa-times"></i></a>
	                            <a href="#" class="on-default edit-row"><i class="fa fa-pencil"></i></a>
	                            <a href="#" class="on-default remove-row" id="delete-{{$component->id}}" onclick="return deleteData({{$component->id}})"><i class="fa fa-trash-o"></i></a>
	                        </td>
	                    </tr>
	                   @endforeach
	                </tbody>
...
function saveData(id)
{
	...
	var url = "http://192.168.0.27/loom/public/component/" +id;
	var name = $('#data_0').val();
	var default_qty = $('#data_1').val();
	var status = $('#data_2').val();
				 $.ajax({
				type: "POST",
				headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
				url: url,
				data: {name:name, default_qty: default_qty, status: status, _method:"PUT"},
...
/* Pull the values and write ajax here */
	var url = "http://192.168.0.27/loom/public/component";
	var name = $('#data_0').val();
	var default_qty = $('#data_1').val();
	var status = $('#data_2').val();
if((name == '') || (default_qty == '') || (status == ''))  {
		return false;
     }
	var data = {name:name, default_qty: default_qty, status: status, _method:"POST"};
				 $.ajax({
				type: "POST",
				headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
				url: url,
